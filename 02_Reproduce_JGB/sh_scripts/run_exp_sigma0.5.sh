...
echo "Usage: bash sh_scripts/run_exp_sigma0.5.sh [USE_NBODY](optional)"
...
echo
echo "Start evolution with gyrFalcON for 13.7 Gyr for: sigma=0.5"
...
python postprocess_snap.py \
  --snap-file snap_mu0.0_s1.0_sigma0.5_r10.0_N20000/out.nemo \
  --remove-point-source \
  --source-mass 4.37e10
...
if [[ $use_nbody -eq 1 ]]; then
  echo
  echo "Preprocess data..."
  snapscale in=snap_mu0.0_s1.0_sigma0.5_r10.0_N20000/IC_preprocessed.nemo \
    out=snap_mu0.0_s1.0_sigma0.5_r10.0_N20000/IC_preprocessed_nbody.nemo \
    mscale=4.300451321727918e-06

  echo
  echo "Start evolution with NBODY0 for 13.7 Gyr for: sigma=0.5"
  nice -n 20 nbody0 snap_mu0.0_s1.0_sigma0.5_r10.0_N20000/IC_preprocessed_nbody.nemo \
    snap_mu0.0_s1.0_sigma0.5_r10.0_N20000/out_nbody.nemo \
    tcrit=14 \
    deltat=0.01 \
    eps=0.0003684031498640387
  ...
  echo "Postprocess data:"
  python postprocess_snap.py \
    --snap-file snap_mu0.0_s1.0_sigma0.5_r10.0_N20000/out_nbody.nemo \
    --nbody \
    --remove-point-source \
    --source-mass 4.37e10
